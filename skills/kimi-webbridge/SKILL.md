...
## ⚠️ Preferencia del usuario — SIEMPRE usar esta skill

**Kimi WebBridge es la herramienta de navegador preferida.** Nunca usar `browser_vision` del sistema de herramientas de Hermes. Siempre usar Kimi WebBridge para screenshots, navegación, y cualquier interacción visual con el navegador.

**Config recommendation:** desactivar `vision` en `agent.disabled_toolsets` para que no aparezca como opción:
```bash
hermes config set agent.disabled_toolsets '["vision"]'
# O editar config.yaml directamente y poner:
# agent:
#   disabled_toolsets:
#   - vision
```

La extensión funciona en Chrome y Edge. Verificar que esté conectada con `status` antes de usar.

## ⚠️ Windows Setup (WSL2)

El daemon corre en **WSL2 Ubuntu** (el binario es Linux-only, no hay build para Windows). Un wrapper en `~/.kimi-webbridge/bin/kimi-webbridge` (Windows) redirige todo a WSL. Los comandos `curl` a `http://127.0.0.1:10086` funcionan directo desde Git Bash/Windows gracias al localhost forwarding automático de WSL2 (incluyendo WebSocket, que la extensión Chrome usa).

- **Start daemon:** `~/.kimi-webbridge/bin/kimi-webbridge start` (usa `--addr 0.0.0.0:10086` SOLO si localhost forwarding no funciona; por defecto `127.0.0.1:10086` funciona con WSL2)
- **Stop daemon:** `~/.kimi-webbridge/bin/kimi-webbridge stop`
- **Check status:** `~/.kimi-webbridge/bin/kimi-webbridge status`
- **Si `extension_connected: false`:** puede necesitar click en la extensión Chrome para reconectar, o reiniciar el daemon

Si el daemon no responde, verificar que WSL2 Ubuntu esté corriendo: `wsl -d Ubuntu -- bash -c "~/.kimi-webbridge/bin/kimi-webbridge status"`

NOTA: NO usar `--addr 0.0.0.0` a menos que sea estrictamente necesario. Con WSL2 moderno, el localhost forwarding (`127.0.0.1` dentro de WSL = `localhost` en Windows) funciona para HTTP y WebSocket.
...
### Pitfall: `fill` fails with "Uncaught" on some textareas

Some frameworks (React controlled textareas, Next.js apps with synthetic event systems) cause `fill` to throw `{"code":"extension_error","message":"fill: Uncaught"}` even on plain `<textarea>` elements.

**Workaround:** use `evaluate` with `execCommand`:

```bash
curl ... -d "{\"action\":\"evaluate\",\"args\":{\"code\":\"(function(){ const el = document.querySelector('textarea'); el.focus(); document.execCommand('selectAll'); document.execCommand('insertText', false, 'YOUR TEXT'); el.dispatchEvent(new Event('input', {bubbles: true})); return 'done'; })()\"},\"session\":\"...\"}"
```

Always wrap in IIFE `(function(){ ... })()` — bare `return` at top level throws `SyntaxError: Illegal return statement`.
...
- **`evaluate` parameter is `code`, not `script`.** Passing `{"script": "..."}` silently fails with "code is required". Always use `{"code": "..."}`.
- **`navigate` resets the JS context.** Any variables, interceptors, or monkey-patches set via `evaluate` are lost after `navigate`. For persistent state, write to `localStorage` before navigating and read it back after.
- **`click` on elements that open modals/confirm dialogs may timeout.** When a `window.confirm()` or modal overlay appears, `snapshot` can hang (60s timeout). Recovery: call `navigate` to the same URL to reset the page state, then check if the action went through.
- **`snapshot` refs (`@e`) are ephemeral.** After any action that changes the DOM (click, fill, navigate), old refs are stale. Always take a new snapshot before using refs from a previous one.
