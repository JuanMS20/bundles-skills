---
name: supabase
description: "Use when doing ANY task involving Supabase. Triggers: Supabase project, .env with VITE_SUPABASE, supabase/ folder, Edge Functions, RLS policies, PostgREST, database migrations, realtime, auth, storage, security audit fixes, API views, column exposure. See references/novavalle-security-fixes.md and references/supabase-security-hardening.md for case studies."
metadata:
  author: supabase
  version: "0.1.2"
---
...
## Security Triggers — Defense-in-Depth Pattern

When RLS alone isn't enough (e.g., service_role key bypasses RLS, or you need server-side enforcement), add PostgreSQL triggers as defense-in-depth.

**Pattern: prevent unauthorized role changes**
```sql
...
**Pattern: prevent orphan admin profiles**
```sql
...
create trigger trg_prevent_orphan_admin
  ...
  for each row execute function prevent_orphan_admin_profiles();
```

**Key rules:**
- Always use `security definer` so the trigger runs with owner privileges
- Always use `set search_path = public` to prevent search_path injection
- Use `old.role is distinct from new.role` (not `!=`) to handle NULLs correctly
- These triggers fire INDEPENDENTLY of RLS — they're a second layer of defense
- Full migration SQL: [references/security-triggers-migration.sql](references/security-triggers-migration.sql)

## Security Pitfalls (must-read before any code)

> **Detailed examples and verification queries:** See `references/security-pitfalls.md`

1. **Never call `auth.admin` from the frontend.** GoTrue Admin API requires `service_role` key. Use Edge Functions or server-side code.

2. **SECURITY DEFINER functions are callable by `anon` by default.** Always revoke and re-grant:
   ```sql
...
4. **Supabase advisor caches results.** Verify actual grants via `information_schema.routine_privileges`.

5. **Testing Edge Functions via browser is unreliable.** Check DB or `get_logs(service='edge-function')` instead.
...
- **RLS, views, and privileged database code**
  ...
  - **RLS policies that reference their own table cause infinite recursion (ERROR 42P17).** If a policy on table X contains a subquery, JOIN, or function call that queries table X (even indirectly through another table), Postgres detects the cycle and blocks all queries with `infinite recursion detected in policy for relation "X"`. `SECURITY DEFINER` on the helper function does NOT break the cycle — Postgres detects the reference at the policy level. This manifests as login succeeding but profile fetch silently failing. **Fix:** cache identity data (role, profile_id) in `auth.users.raw_app_meta_data` (no RLS), write helper functions that read from `auth.users`, and rewrite all policies to use these helpers instead of querying the protected table. See `references/security-pitfalls.md` for the full fix pattern.
...
- **Supabase Security Hardening** → [references/supabase-security-hardening.md](references/supabase-security-hardening.md) — REVOKE EXECUTE on SECURITY DEFINER functions, SET search_path
